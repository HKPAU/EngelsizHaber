...
if(isset($_GET["ID"])){
    $GelenID    = FilterForNumber($_GET["ID"]);
}else{
    $GelenID    = "";
}
    ...
    <tr>
        <td>
            <table style="width:100% ; background-color : #E4A27E">
                <tr>
                    <td><img src="Images/<?php echo $GazeteBilgisi["GazeteLogo"]; ?>" alt=""> </td>
                    <td align="right" style="font-size : 74px ; color : #004447 ; font-family : Gil-Sans"><?php echo $GazeteBilgisi["GazeteAdi"]; ?> Gazetesi Haberleri</td>
                </tr>
            </table>
        </td>
    </tr>
    ...
    <tr>
        <td style="background-color : #F9F4D7 ; padding:10px">
            <table>
                ...
                $Limit                  = 5;
                $Gosterim               = 1;
                $SabahHaberleriOku      = $db->prepare("SELECT * FROM Haberler WHERE GazeteID = ? ORDER BY EklenmeTarihi DESC");
                $SabahHaberleriOku->execute([$GelenID]);
                $Control2               = $SabahHaberleriOku->rowCount();



                if($Control2 > 0){
                    $Icerikler  = $SabahHaberleriOku->fetchAll();
